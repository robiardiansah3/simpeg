    ...
    <x-slot name="trigger">
        @if ($position === UserMenuPosition::Topbar)
            <button
                ...
                type="button"
                class="fi-user-menu-trigger flex items-center gap-x-2.5 rounded-xl border border-gray-200 dark:border-white/10 transition hover:bg-gray-50 dark:hover:bg-white/5"
                style="border: 1px solid #E2E8F0; border-radius: 10px; padding: 6px 10px 6px 6px; display: flex; align-items: center; gap: 10px; cursor: pointer; transition: background-color 150ms;"
            >
                ...
                <div class="flex flex-col text-left">
                    <span style="font-size: 13px; font-weight: 600; color: #1E293B; line-height: 1.3; white-space: nowrap;">
                        {{ filament()->getUserName($user) }}
                    </span>
                    <span style="font-size: 11px; color: #64748B; line-height: 1.3; white-space: nowrap;">
                        {{ $user->role?->display_name ?? ($user->role?->role_name ?? 'Administrator') }}
                    </span>
                </div>
            </button>
        @else
